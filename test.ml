...
let map2_test n =
  ...
  Command_unix.run
    (Bench.make_command
      [
        Bench.Test.create ~name:(Printf.sprintf "map2 trmc   %d" n) (fun () -> g := map2 (+) lst lst);
...
let mapi_test n =
  ...
  Command_unix.run
    (Bench.make_command
      [
        Bench.Test.create ~name:(Printf.sprintf "mapi trmc   %d" n) (fun () -> g := mapi (+) lst);
...
let map_test n =
  ...
  Command_unix.run
    (Bench.make_command
      [
        Bench.Test.create ~name:(Printf.sprintf "map trmc   %d" n) (fun () -> g := map succ lst);
